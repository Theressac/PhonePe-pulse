...
# Selection option
option = st.radio('**Select an option**', ('All India', 'State wise', 'Top Ten categories'), horizontal=True)
...
# SQL Query to fetch Tranaction Analysis details
        ...
        trans_qry_rslt = myCursor.fetchall()
        ...
        trans_analy_rslt = myCursor.fetchall()
        ...
        Trans_amt_qry_rslt = myCursor.fetchall()
        ...
        trans_co_qry_rslt = myCursor.fetchall()
    ...
    with tab2:
        ...
        myCursor.execute(
            f"SELECT state, SUM(user_count) FROM aggregated_users WHERE year = '{in_us_yr}' AND quarter = '{in_us_qtr}' GROUP BY state;")
        ...
        myCursor.execute(
            f"SELECT SUM(user_count), AVG(user_count) FROM aggregated_users WHERE year = '{in_us_yr}' AND quarter = '{in_us_qtr}';")
...
# ==============================================          /     State wise       /             ============================================== #
    ...
    with tab4:
        ...
        # User Analysis Bar chart query
        myCursor.execute(
            f"SELECT quarter, SUM(user_count) FROM aggregated_users WHERE state = '{st_us_st}' AND year = '{st_us_yr}' GROUP BY quarter;")
        ...
        # Total User Count table query
        myCursor.execute(
            f"SELECT SUM(user_count), AVG(user_count) FROM aggregated_users WHERE state = '{st_us_st}' AND year = '{st_us_yr}';")
...
# ==============================================          /     Top categories       /             =========================================== #
    ...
    with tab6:
        ...
        myCursor.execute(
            f"SELECT state, SUM(registered_user) AS Top_user FROM top_users WHERE year='{top_us_yr}' GROUP BY state ORDER BY Top_user DESC LIMIT 10;")
